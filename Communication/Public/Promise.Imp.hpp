...
Future< Result > Future< T >::_then(Resolve&& resolve, Reject&& reject) {
	...
	typedef std::function< void(const PromiseError&) > RejectHandler;
	RejectHandler rejectHandler;
	if (std::is_same< std::nullptr_t, Reject >::value) {
		rejectHandler = [next](const PromiseError& err) {
			next->reject(err);
		};
	}
	else {
...
void Future< T >::then(promise_type&& promise) {
	...
	m_state->reject = [prev, next](const PromiseError& err) mutable {
		next->reject(err);
		prev->resolve = nullptr;
		prev.reset();
	};
