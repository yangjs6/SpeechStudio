...
public:
	PromiseError()
		: PromiseError(nullptr,0)
	{
	}

	explicit PromiseError(TCHAR const* const InMessage)
		: PromiseError(InMessage,0)
	{
	}

	PromiseError(TCHAR const* const InMessage, int InErrorCode) throw()
		: ErrorMessage(InMessage), ErrorCode(InErrorCode)
	{
	}

	PromiseError(PromiseError const& _Other)
		: ErrorMessage(_Other.ErrorMessage), ErrorCode(_Other.ErrorCode)
	{
	}

	PromiseError& operator=(PromiseError const& _Other) = delete;


	virtual TCHAR const* What() const
	{
		return ErrorMessage ? ErrorMessage : TEXT("Unknown Error");
	}
...
private:
	...
	struct _SharedState {
		...
		std::function<void(const PromiseError&)> reject;
	};
