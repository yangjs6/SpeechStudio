...
void UMP3Decoder::Init(const TArray<uint8>& InBuffer)
{
	const uint8* Buffer = InBuffer.GetData();
	const int BufferSize = InBuffer.Num();
	...
	mpg123_init();
	Handle = mpg123_new(nullptr, &ErrorHandle);
	BlockBufferSize = mpg123_outblock(Handle);

	mpg123_open_feed(Handle);
	mpg123_feed(Handle, Buffer, BufferSize);
	mpg123_getformat(Handle, &samplerate, &channels, &encoding);
	const uint32 bytesPerSample = mpg123_encsize(encoding);
...
bool UMP3Decoder::Decode(TArray<uint8> &OutBuffer)
{
	...
	unsigned char* BlockBuffer = static_cast<unsigned char*>(FMemory::Malloc(BlockBufferSize));
	size_t bytesRead = 0;
	size_t done = 0;
	int result;
...
void UMP3Decoder::WriteWaveHeader(uint8* header, uint32 dataLength, uint32 sampleRate, uint32 channels)
{
	...
	FMemory::Memcpy(header + 32, &buff16, 2); //<block align> Frame-Groese = <Anzahl der Kan�le> * ((<Bits/Sample (eines Kanals)> * 7) / 8) 
